...
public class RecipeScrappingRunner extends BaseClass {
	...
	// Database Password
	public static String DB_PASSWORD = "postgress";
...
//            for (int pageNum = 1; pageNum <= numberOfPages; pageNum++) {
		...
		for (RecipeInformation recipeInformation : recipeInformationMap.values()) {
...
//        recipeInformationMap.values().stream().map(x -> x.getIngredients().contains())
	...
	//LFV
	@Test
	public void startScraping_LFV() throws InterruptedException, IOException {

		List<RecipeInformation> eliminatorRecipeInformation = new ArrayList<>();
		List<RecipeInformation> toAddRecipeInformation = new ArrayList<>();
		Map<String, List<String>> eliminators_ToAdd_Map = ReadExcel.readExcelDataLFVElimination();
		System.out.println("started scrapping lfv");
		clickAToZ();
...
//        recipeInformationMap.values().stream().map(x -> x.getIngredients().contains())
		...
		eliminatorRecipeInformation.stream().forEach(recipeInformation -> {
			try {
				insertLFVDataEliminatorTable(recipeInformation);
		...
		toAddRecipeInformation.stream().forEach(recipeInformation -> {
			try {
				insertLFVaddTable(recipeInformation);
	...
	public void insertDataEliminatorTable(RecipeInformation recipeInformation) throws SQLException {
		String sql = "INSERT INTO lfv_eliminators(\n" + "  recipe_id,\n" + "  recipe_name,\n" + "  recipe_category,\n"
				+ "  food_category,\n" + "  ingredients,\n" + "  preparation_time,\n" + "  cooking_time,\n"
				+ "  preparation_method,\n" + "  nutrient_values,\n" + "  recipe_url\n" + ") VALUES ("
				+ recipeInformation.getRecipeID() + ",'" + recipeInformation.getRecipeName() + "','"
	...
	public void insertLFVDataEliminatorTable(RecipeInformation recipeInformation) throws SQLException {
		String sql = "INSERT INTO lfv_eliminator_recipe(\n" + "  recipe_id,\n" + "  recipe_name,\n" + "  recipe_category,\n"
				+ "  food_category,\n" + "  ingredients,\n" + "  preparation_time,\n" + "  cooking_time,\n"
				+ "  preparation_method,\n" + "  nutrient_values,\n" + "  recipe_url\n" + ") VALUES ("
				+ recipeInformation.getRecipeID() + ",'" + recipeInformation.getRecipeName() + "','"
				+ recipeInformation.getRecipeCategory() + "','" + recipeInformation.getFoodCategory() + "','"
	...
	public void insertLFVaddTable(RecipeInformation recipeInformation) throws SQLException {
		String sql = "INSERT INTO lfv_add_recipe(\n" + "  recipe_id,\n" + "  recipe_name,\n" + "  recipe_category,\n"
				+ "  food_category,\n" + "  ingredients,\n" + "  preparation_time,\n" + "  cooking_time,\n"
				+ "  preparation_method,\n" + "  nutrient_values,\n" + "  recipe_url\n" + ") VALUES ("
				+ recipeInformation.getRecipeID() + ",'" + recipeInformation.getRecipeName() + "','"
				+ recipeInformation.getRecipeCategory() + "','" + recipeInformation.getFoodCategory() + "','"
	...
	public void navigateAlphabetAndPage(String alphabet, Integer pageNumber) {
		driver.navigate().to(AppConstants.urlByAphabet.concat(alphabet).concat(AppConstants.urlPageIndex)
	...
	public void setRecipeCategory(RecipeInformation recipeInformation, String tagName, String recipeDescription) {
		if (tagName.toLowerCase().contains(RecipeCategory.BREAKFAST.toString().toLowerCase())
				|| recipeDescription.toLowerCase().contains(RecipeCategory.BREAKFAST.toString().toLowerCase())) {
			recipeInformation.setRecipeCategory(RecipeCategory.BREAKFAST);
		} else if (tagName.toLowerCase().contains(RecipeCategory.DINNER.toString().toLowerCase())
				|| recipeDescription.toLowerCase().contains(RecipeCategory.DINNER.toString().toLowerCase())) {
			recipeInformation.setRecipeCategory(RecipeCategory.DINNER);
		} else if (tagName.toLowerCase().contains(RecipeCategory.LUNCH.toString().toLowerCase())
				|| recipeDescription.toLowerCase().contains(RecipeCategory.LUNCH.toString().toLowerCase())) {
			recipeInformation.setRecipeCategory(RecipeCategory.LUNCH);
		} else if (tagName.toLowerCase().contains(RecipeCategory.SNACK.toString().toLowerCase())
				|| recipeDescription.toLowerCase().contains(RecipeCategory.SNACK.toString().toLowerCase())) {
